...
classdef imtool3DROI_line < imtool3DROI
    ...
    properties (SetAccess = protected, GetAccess = protected)
        ...
        pheight      % height of the line profile drawn above the ROI line. Default sets it to a percentage of the 
                     % size of the image
    end
    ...
    methods
        ...
        function ROI = imtool3DROI_line(varargin)
            ...
            menuLabels = {'Export stats', 'Plot profile', 'Hide Text', 'Hide profile', 'Delete', ...
                'Symmetrize mask (current slice)', 'Symmetrize mask (all slices)', 'Split mask (current slice)', ...
                'Split mask (all slices)'};
            ...
            ROI.textHandle = text(x, y, 'text', 'Parent', parent, 'Color', 'w', 'FontSize', 10, 'EdgeColor', 'w', ...
                'BackgroundColor', 'k', 'HorizontalAlignment', 'Left', 'VerticalAlignment', 'top', 'Clipping', 'on');
            hold on;
            graphicsHandles(3) = plot(position(:, 1), position(:, 2)-ROI.pbuff, '-r', 'LineWidth', 1.5, ...
                'Parent', parent);
            set(parent, 'NextPlot', nextPlot);
...
function contextMenuCallback(source, ~, ROI, tool, islct)
    switch get(source, 'Label')
        ...
        case 'Symmetrize mask (current slice)'
            ...
            % save
            ind = sub2ind(size(Mask), min(size(Mask, 1), max(1, round(UV_Sym(:, 2)))), min(size(Mask, 2), ...
                max(1, round(UV_Sym(:, 1)))));
            Mask(ind) = true;
