...
classdef imtool3D_3planes < handle
    ...
    methods
        ...
        function tool3P = imtool3D_3planes(varargin)    % Constructor (dat, mask, parent, range)
            ...
            if(isempty(parent))
                parent = tool(1).getHandles.fig;
                resizefactorthree = true;
            end
            ...
            for(ic = 1:length(controls1))
                if(~isempty(controls1(ic).Callback) && contains(func2str(controls1(ic).Callback), 'saveImage'))
                    continue
                end % save Mask only once!
                
                if(~isempty(controls1(ic).Callback) && contains(func2str(controls1(ic).Callback), 'displayHelp'))
                    continue
                end % Display Help only once!
                ...
                if(strcmp(get(controls1(ic), 'Style'), 'togglebutton'))
                    CB2 = @(h, e) cellfun(@(x) feval(x, h, e), {@(h, e) set(controls2(ic), 'Value', ...
                        ~get(controls2(ic), 'Value')), CB2});
                    CB3 = @(h, e) cellfun(@(x) feval(x, h, e), {@(h, e) set(controls3(ic), 'Value', ...
                        ~get(controls3(ic), 'Value')), CB3});
                end
            ...
            % preallocate ContrastWDBF cell array
            ContrastWBDF = cell(1,3);
            for(ii=1:3)
                ContrastWBDF{ii} = get(tool(ii).getHandles.I(1), 'ButtonDownFcn');
            end
            txt = uicontrol(hp, 'Style', 'text', 'String', 'Mouse Left Click:', 'Units', 'Pixels', ...
                'Position', [5, 0, 100, 20]);
            ...
            lbo2 = uicontrol(lbo, 'Style', 'radiobutton', 'String', 'Cursor (hold [X] key)', ...
                'Position', [105, 0, 150, 20]);
    ...
    switch(event.Key)
        ...
        otherwise
            ...
            if(~isequal(CB_Motion_mod{1}, CB_Motion_mod{2}))
                set(fig, 'WindowButtonMotionFcn', @(src, evnt) Callback3(CB_Motion_mod{1}, CB_Motion_mod{2}, ...
                    CB_Motion_mod{3}, src, evnt));
            end
    end
end
    ...
    for(ii = 1:3)
        switch(get(event.NewValue, 'String'))
            ...
            case 'Cursor (hold [X] key)'
                set(tool(ii).getHandles.I, 'ButtonDownFcn', ...
                    @(src, evnt) BTF_syncSlices(src, evnt, ContrastWBDF{ii}, tool3P));
                set(tool(ii).getHandles.mask, 'ButtonDownFcn', ...
                    @(src, evnt) BTF_syncSlices(src, evnt, ContrastWBDF{ii}, tool3P));
        end
    end
end
...
function showhelpannotation(tool3P)
    ...
    if(get(tool.getHandles.Tools.Help, 'Value'))
        a = rectangle(tool(1).getHandles.Axes(end), 'Position', [0 0 1e12 1e12], 'FaceColor', [0 0 0 0.7]);
        ...
        a(4) = annotation(tool(3).getHandles.Panels.Image, 'textarrow', [0.9 1], [0.85 0.85], ...
            'String', sprintf(['3D Mask (Multi-label ROI)\n- Select label number\n- Fill with Paint brush\n', ...
            '- Hover label for stats\n- Right-click to delete']), 'Color', [1 1 1]);
        a(5) = annotation(tool(1).getHandles.Panels.Image, 'textarrow', [0.9 1], [0.25 0.25], ...
        ...
        a(8) = annotation(tool(1).getHandles.Panels.Image, 'textarrow', [0.1 0], [0.5 0.5], ...
            'String', sprintf('Use scroll wheel to navigate\nthrough slices'), 'Color', [1 1 1], ...
            'HorizontalAlignment', 'left');
        a(9) = annotation(tool(2).getHandles.Panels.Image, 'textbox', [0.05 0.1 0.9 0.4], 'FitBoxToText', 'off', ...
            ...
            'HorizontalAlignment', 'left');
        a(10) = rectangle(tool(2).getHandles.Axes(end), 'Position', [0 0 1e12 1e12], 'FaceColor', [0 0 0 0.7]);
        a(11) = rectangle(tool(3).getHandles.Axes(end), 'Position', [0 0 1e12 1e12], 'FaceColor', [0 0 0 0.7]);
        a(12) = annotation(tool(2).getHandles.Panels.Image, 'textarrow', [0.5 0.5], [0.8 1], ...
            'HorizontalAlignment', 'left', 'String', sprintf(['Three ways to go through slices:\n- use scroll ', ...
            'wheel on each slicing plane\n- Position cursor on the image and press the [X] key.\n- Select ', ...
            'cursor mode here and click on the image.']), 'Color', [1 1 1]);
