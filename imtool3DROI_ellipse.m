...
classdef imtool3DROI_ellipse < imtool3DROI_rect
    ...
    methods
        ...
        function ROI = imtool3DROI_ellipse(varargin)
            ...
            % TODO: Figure out what exactly the @ does in the following line and document it. This syntax is
            % unfamiliar. I'm familiar with the use of @ to create function handles, but it's not clear to me what
            % putting ROI immediately before the @imtool3DROI_rect does.
            ...
            for(i = 1:length(ROI.graphicsHandles))
                ...
                set(ROI.graphicsHandles(i), 'ButtonDownFcn', fun);
            end
        end
        ...
        function success = autoCenterROIFindCircleMethod(ROI, circleRange)
            ...
            center = imfindcircles(im, round(rescale*circleRange), 'Sensitivity', 0.95);
...
function ButtonMotionFunction(~, ~, ROI, n)
    ...
    switch(n)
        ...
        case 3	% top left corner
            ...
            dx = x-cp(1);
            dy = y-cp(2);
            cp(1) = position(1)-(position(3)/2+dx);
            cp(2) = position(2)-(position(4)/2+dy);
