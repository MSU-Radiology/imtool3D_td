...
classdef imtool3DROI_rect < imtool3DROI
    ...
    methods
        ...
        function ROI = imtool3DROI_rect(varargin)
            ...
            graphicsHandles(1) = rectangle('Position', pos, 'Parent', parent, 'EdgeColor', 'r', 'LineWidth', 1.5, ...
                'Curvature', [0 0]);
            ...
            % middle cross
            graphicsHandles(2) = plot(position(1), position(2), '+r', 'MarkerSize', 12, 'Parent', parent);
            
            % top left corner
            graphicsHandles(3) = plot(pos(1), pos(2), 'sr', 'MarkerSize', 8, 'Parent', parent, 'MarkerFaceColor', 'r');
            
            % top right corner
            graphicsHandles(4) = plot(pos(1)+pos(3), pos(2), 'sr', 'MarkerSize', 8, 'Parent', parent, ...
                'MarkerFaceColor', 'r');
            
            % bottom left corner
            graphicsHandles(5) = plot(pos(1), pos(2)+pos(4), 'sr', 'MarkerSize', 8, 'Parent', parent, ...
                'MarkerFaceColor', 'r');
            
            % bottom right corner
            graphicsHandles(6) = plot(pos(1)+pos(3), pos(2)+pos(4), 'sr', 'MarkerSize', 8, 'Parent', parent, ...
                'MarkerFaceColor', 'r');
            
            % left
            graphicsHandles(7) = plot(pos(1), position(2), 'sr', 'MarkerSize', 8, 'Parent', parent, ...
                'MarkerFaceColor', 'r');
            
            % right
            graphicsHandles(8) = plot(pos(1)+pos(3), position(2), 'sr', 'MarkerSize', 8, 'Parent', parent, ...
                'MarkerFaceColor', 'r');
            
            % top
            graphicsHandles(9) = plot(position(1), pos(2), 'sr', 'MarkerSize', 8, 'Parent', parent, 'MarkerFaceColor', 'r');
            
            % bottom
            graphicsHandles(10) = plot(position(1), pos(2)+pos(4), 'sr', 'MarkerSize', 8, 'Parent', parent, ...
                'MarkerFaceColor', 'r');
            ...
            if(~exist('tool', 'var') || isempty(tool))
                menuLabels(end-1:end) = [];
                tool = [];
            end
            ...
            ROI.textHandle = text(pos(1), pos(2)-ROI.tbuff, 'text', 'Parent', parent, 'Color', 'w', 'FontSize', 10, ...
                'EdgeColor', 'w', 'BackgroundColor', 'k', 'HorizontalAlignment', 'Left', ...
...
function contextMenuCallback(source, ~, ROI, tool)
    switch(get(source, 'Label'))
        ...
        case 'crop'
            ...
            x = max(1, round(x));
            y = max(1, round(y));
            I = cellfun(@(X) X(min(end, min(y)):min(end, max(y)), min(end, min(x)):min(end, max(x)), :, :, :, :), ...
                I, 'uni', 0);
            tool.setImage(I);
            delete(ROI);
    end
