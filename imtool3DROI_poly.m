...
classdef imtool3DROI_poly < imtool3DROI
    ...
    methods
        ...
        function ROI = imtool3DROI_poly(varargin)
            ...
            graphicsHandles(2) = plot(position(:, 1), position(:, 2), 'sr', 'LineWidth', 1.5, 'MarkerSize', 8, ...
                'Parent', parent, 'MarkerFaceColor', 'r');
            % centroid cross
            graphicsHandles(3) = plot(position(1, 1), position(1, 2), '+r', 'MarkerSize', 12, 'Parent', parent);
            graphicsHandles(4) = plot(0, 0, 'oy', 'LineWidth', 1.5, 'MarkerSize', 8, 'Parent', parent, ...
                'MarkerFaceColor', 'r');
            ...
            ROI.tbuff = 0.02*size(I, 1);
            [x, y] = findTextPosition(position, ROI.tbuff);
            ROI.textHandle = text(x, y, 'text', 'Parent', parent, 'Color', 'w', 'FontSize', 10, 'EdgeColor', 'w', ...
                'BackgroundColor', 'k', 'HorizontalAlignment', 'Left', 'VerticalAlignment', 'bottom', 'Clipping', 'on');
            ...
            for(i = 1:length(graphicsHandles))
                fun = @(hObject, evnt) ButtonDownFunction(hObject, evnt, ROI, i);
                set(graphicsHandles(i), 'ButtonDownFcn', fun);
            end
        ...
        function newPosition(ROI, markerPosition)
            ...
            elseif(all(ROI.curveindex))
                ...
                ROI.position = ppval(pp, unique([linspace(tt(Npts+1), tt(2*Npts+1), ...
                    sum(markerDist)/3/3), tt(Npts+1:2*Npts+1)]))';
            else
            ...
            set(graphicsHandles(2), 'XData', ROI.markerPosition(~curveindexext, 1), ...
                'YData', ROI.markerPosition(~curveindexext, 2));
            set(graphicsHandles(4), 'XData', ROI.markerPosition(curveindexext, 1), ...
                'YData', ROI.markerPosition(curveindexext, 2));
...
function [x, y] = getPolygonCentroid(position)
    ...
    xi = position(1:end-1, 1);
    xip1 = position(2:end, 1);
    yi = position(1:end-1, 2);
    yip1 = position(2:end, 2);
    xiyip1 = xi.*yip1;
    xip1yi = xip1.*yi;
    diff = xiyip1-xip1yi;
    
    % Get area
    A = 0.5 .* sum(xiyip1 - xip1yi);
    A = 1./(6*A);
...
function ButtonDownFunction(~, evnt, ROI, n)
    ...
    op = get(ROI.axesHandle, 'CurrentPoint');
    op=[op(1, 1), op(1, 2)];
    ...
    if(n == 2 || n == 4)  % User clicked on a vertice to move it (or delete it)
        ...
        if(strcmp(click, 'extend') && ((~isopen && size(position_old, 1) > 4) || ...
                (isopen && size(position_old, 1) > 2))) % user wants to delete the vertice
            switch(ind)
                case 1
    ...
    if((n ==1) && strcmp(click, 'normal'))    % The user clicked on a line and wants to add a vertice
        ...
        markertype = ROI.curveindex(1+mod(ind-1, length(ROI.curveindex))) | ...
            ROI.curveindex(1+mod(ind-1+1, length(ROI.curveindex)));
...
function ButtonMotionFunction(~, ~, ROI, n, op, position_old, ind)
    cp = get(ROI.axesHandle, 'CurrentPoint');
    cp=[cp(1, 1), cp(1, 2)];
    isopen = ~isequal(position_old(end, :), position_old(1, :));
    switch(n)
        ...
        otherwise
            % location remains unchanged (happens when a vertice is removed and the user keeps the button pressed 
        ...
        case 'Export stats'
            ...
            try
                ...
                if(strcmp(idSegLast, 'assigninInvalidVariable'))
                    warndlg({'Invalid variable name.'; 'Stats were not exported to the workspace.'}, ...
