...
function imtool3D_nii_crop(filename)
    ...
    if(~isdeployed)
        if(~exist('filename','var'))
            help imtool3D_nii_crop;
            return
        end
        
        if(~exist(filename,'file'))
            error(['can''t find nifti file ' filename]);
        end
    ...
    qmod = R*[cut_from_L*nii.hdr.pixdim(2); ...
        cut_from_P*nii.hdr.pixdim(3); ...
        cut_from_I*nii.hdr.pixdim(4)*nii.hdr.pixdim(1)];
