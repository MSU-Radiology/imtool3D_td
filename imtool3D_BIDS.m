...
function imtool3D_BIDS(BidsFolder)
    ...
    ptool = uipanel(h);
    ptool.Position = [0, 0, 1, 0.8];
    plb = uipanel(h);
    plb.Position = [0, 0.8, 1, 0.2];
    % VIEWER
    ...
    % Add view button
    btn_view = uicontrol(plb, 'Style', 'pushbutton', 'String', 'view', 'Units', 'normalized', ...
        'Position', [0.9, 0, 0.1, 0.3], 'BackgroundColor', [0, 0.65, 1]);
...
function filterDatabase(BIDS, tsub, tses, tmodality, tsequence, listbox)
    ...
    if(strcmp(listbox, 'sub') || strcmp(listbox, 'ses'))
        tmodality.String = unique(bids.query(BIDS, 'modalities', 'sub', tsub.String(tsub.Value), ...
            'ses', tses.String(tses.Value)));
        tmodality.Value(tmodality.Value > length(tmodality.String)) = [];
    end
    
    if(isempty(tses.String(tses.Value))) % no sessions
        tsequence.String = bids.query(BIDS, 'types', 'sub', tsub.String(tsub.Value), ...
            'modality', tmodality.String(tmodality.Value));
    else
        tsequence.String = bids.query(BIDS, 'types', 'sub', tsub.String(tsub.Value), 'ses', tses.String(tses.Value), ...
            'modality', tmodality.String(tmodality.Value));
    end
...
function viewCallback(tool3P, BIDS, tsub, tses, tmodality, tsequence)
    ht = wait_msgbox;
    if(isempty(tses.String(tses.Value))) % no sessions
        dat = bids.query(BIDS, 'data', 'sub', tsub.String(tsub.Value), ...
            'modality', tmodality.String(tmodality.Value), 'type', tsequence.String(tsequence.Value));
    else
        dat = bids.query(BIDS, 'data', 'sub', tsub.String(tsub.Value), 'ses', tses.String(tses.Value), ...
            'modality', tmodality.String(tmodality.Value), 'type', tsequence.String(tsequence.Value));
    end
    if(isempty(dat))
        if(ishandle(ht))
